...
public class StateEditCommand implements ArgumentCommand {
    ...
    public static final OptionalArgument<List<String>> REM_ARGUMENT = new OptionalArgument<>(new RemainingArgument<>("remaining", new StringArgument("temp")), new ArrayList<>());
    ...
    public boolean run(@NotNull CommandContext context, @NotNull String[] arg) {
        ...
        if (attrib.equalsIgnoreCase("name")) {
            ...
            if (newAttrib.length() != 3) {
                context.getSource().sendMessage(ChatColor.BLUE + "[EuroStates] " +
                        ChatColor.RED + "Tags must be 3 letters.");
                return true;
            }
            ...
            if (!(offlinePlayer.hasPlayedBefore())) {
                context.getSource().sendMessage(ChatColor.BLUE + "[EuroStates] " +
                        ChatColor.RED + "User has not played on this server before.");
                return true;
            }
            ...
            if (newPrefix.length() > 21 || newPrefix.length() < 3) {
                context.getSource().sendMessage(ChatColor.BLUE + "[EuroStates] " +
                        ChatColor.RED + "Prefix rank cannot be shorter than 3 or longer than 21.");
                return true;
            }
