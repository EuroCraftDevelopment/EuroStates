...
public class TechnologyPermissionRemoveCommand implements ArgumentCommand {
    ...
    public boolean run(@NotNull CommandContext context, @NotNull String[] arg) {
        ...
        if(!(technology.getPermissions().contains(permission))) {
            context.getSource().sendMessage("It does not have that as a permission dumbass.");
            return true;
        }

        technology.getPermissions().remove(permission);
        try {
            technology.save();
        } catch (IOException e) {
            e.printStackTrace();
        }
        context.getSource().sendMessage("Removed " + permission + " as a permission from "+technology.getIdentifier());
        return true;
    }
}
